	...
	public class ContestService : IContestService
	{
		...
		public async Task<ActionServiceResult> CheckScreen(string userID, string contestID, string roleName)
		{
			...
			else
			{
				...
				if (DateTime.Compare(DateTime.Now, result.StartTime) < 0)
				{
					return new ActionServiceResult(true, "Chưa đến thời gian làm bài", Code.NotTimeToDo, null, 0);
				}

				else if (DateTime.Compare(DateTime.Now, result.FinishTime) > 0)
				{
				...
				else
				{
					...
					{
						contestDTO.ExamID = exam.ExamId;
						if (exam.IsDoing == 1 && DateTime.Compare(DateTime.Now, exam.ModifiedDate.Value) > 0 && exam.Status == 0)
						{
